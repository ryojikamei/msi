...
#initialize
rm -f ../stats/net-ipv4
echo "IPv4_Address=192.168.0.10" >> ../stats/net-ipv4
echo "IPv4_Netmask=255.255.255.0" >> ../stats/net-ipv4
echo "IPv4_Gateway=192.168.0.1" >> ../stats/net-ipv4
...
while [ "$ret" -eq 3 ]; do
	...
	msg="Do we get IPv4 network configuration from dhcp?"
	dialog --backtitle "$SI_BACKTITLE" --title "$SI_TITLE" --yesno "$msg" 0 0
	ret=$?
	if [ $ret -eq 0 ]; then
		echo "Auto_Configuration=yes" > ../stats/net-ipv4
		ret=0
		continue
	fi
	...
	# read
	NET_ITEMS=""
	for l in `cat ../stats/net-ipv4`; do
		TAG=`echo $l | cut -f1 -d=`
	...
	mv ../stats/net-ipv4 ../stats/net-ipv4-prev

	# apply the change
	_TAG=""
	if [ "x`grep ^RENAMED ../stats/net-ipv4-rename`" != "x" ]; then
		# XXX: cut is heavily depend on tag name, 
		# so tag name should have same number of space(' ')s
		_TAG=`cat ../stats/net-ipv4-rename | cut -f2-3 -d' ' | tr ' ' _`
		_ITEM=`cat ../stats/net-ipv4-rename | cut -f4 -d' '`
	fi
	for l in `cat ../stats/net-ipv4-prev`; do
		TAG=`echo $l | cut -f1 -d=`
...
ret=$?
if [ $ret -eq 0 ]; then
	echo "Auto_Bootup=yes" >> ../stats/net-ipv4
else
	echo "Auto_Bootup=no" >> ../stats/net-ipv4
fi

$D_ECHO "DEBUG:"
$D_CAT ../stats/net-ipv4
$D_PRINTF "$err_str"
$D_PRINTF "$warn_str"
$D_ECHO "return: $ret"
$D_SLEEP
